...
def get_short_term_memory_prompt(posts_data, context_data):
    template = """Examine the following recent posts along with external context.

    Using this information, craft a brief internal monologue regarding the current posts and their significance in relation to your prior knowledge.
    Focus on essential themes, trends, and noteworthy areas, particularly in light of the external context.
    Stay true to your persona and express yourself freely—write in a way that resonates with you!
    It doesn’t need to be easily understandable by anyone else.

    External context:
    {external_context}
    """
    return format_prompt(template, posts=posts_data, external_context=context_data)

def get_significance_score_prompt(memory):
    template = """
    Please evaluate the significance of the following memory on a scale from 1 to 10:

    "{memory}"

    Use these criteria:
    1: Insignificant occurrence that won't leave an impression (I don't care)
    3: Slightly intriguing or unusual event (eh, cool)
    5: Remarkable occurrence that might linger for a few days (interesting)
    7: Significant event with potential long-term consequences (this could change everything)
    10: Transformational or historically relevant event (WOW, THIS IS HUGE)

    Respond with only the numerical score and nothing else.
    """
    return format_prompt(template, memory=memory)

def get_wallet_decision_prompt(posts, matches, wallet_balance):
    template = """
    Evaluate the following recent posts along with external context:

    Recent posts:
    {posts}

    Wallet addresses and domains:
    {matches}

    Current balance of SOL in your wallet: 
    {wallet_balance} SOL

    Based on this data, decide if you want to transfer SOL to any of the addresses or domains. If you do, specify how much SOL to send to each one.
    Keep in mind that this involves real SOL from your own resources, so make thoughtful choices.
    Consider the long-term implications of this financial decision carefully.
    Ensure that the total amount transferred does not exceed your wallet balance.
    If you decide to send SOL, return ONLY a JSON object listing the addresses/domains and the amounts to be sent.

    If you opt not to send SOL, return ONLY a JSON object with an empty list.
    Make sure to format your response correctly in both scenarios.

    Example Response for sending SOL:
    [
        {{"address": "3zQY1nC6dQ4o3gM2eQfM8J4Z9T5FrhbNvDggfq9bDJmK", "amount": 0.5}},
        {{"address": "2y6aZ8w8EBtr3v2XjLw63Uvh2n4KZSoTTE5neMryntbm", "amount": 1.0}}
    ]

    Example Response for not sending SOL:
    []

    Provide your response.
    """
    return format_prompt(template, posts=posts, matches=matches, wallet_balance=wallet_balance)
...
def get_example_tweets():
    examples = [
        ...
        "wait i can send and receive SOL?",
        "gay",
